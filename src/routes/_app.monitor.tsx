...
// لازم نفس الـ string ده يتبعت مع طلبات الـ Scapy script على /predict/full
// (كـ device_id) عشان الربط بين الجهاز واليوزر يشتغل صح
const DEVICE_ID = "scapy-monitor-01";
...
function Monitor() {
  ...
  const fetchLive = async () => {
    ...
    if (!userId) return;
    try {
      const res = await fetch(`${API_URL}/api/logs?user_id=${userId}&limit=200`);
      if (!res.ok) return;
      ...
      setLiveRows(
        data.map((r: any) => ({
          ...
          source_country: r.source_country || "Unknown",
